...
void FirstSetUp()
{
	...
	int YCreatingNumber = 2;
	bool Corner = true;

	for (size_t i = 0; i < BUTTONCOUNT; i++)
	{
		int XCreatingNumber = 1;
		...
		Buttons.push_back(RectangleShape({ ButtonSize.x * XCreatingNumber, ButtonSize.y }));
		Buttons[i].setFillColor(ButtonColor);

		if (Corner)
		{
			Buttons[i].setPosition(0.f, YCreatingNumber * 100.f); Corner = false;
		}
		else
		{
			Buttons[i].setPosition(Buttons[i - 1].getPosition().x + Buttons[i - 1].getSize().x, YCreatingNumber * 100.f);
		}
